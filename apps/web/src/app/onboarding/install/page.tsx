      ...
      <div className="w-full max-w-lg">
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          ...
          <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
            Install the RunSpend GitHub App on the org you want to analyze.
            We&apos;ll only read workflow run metadata — no source, no logs, no
            secrets.
          </p>
...
function StepLine({ done }: { done?: boolean }) {
  return (
    <span
      className={`mx-2 h-px flex-1 ${done ? "bg-emerald-500/50" : "bg-border"}`}
    />
  );
}
